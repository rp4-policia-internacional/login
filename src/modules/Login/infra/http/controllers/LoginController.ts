...
export default class LoginController{
    public async create(req: Request, res: Response): Promise<Response> {
        ...
        return res.json(createdLogin).status(201).send("Produto criado");
    }
    ...
    public async delete(req: Request, res: Response): Promise<Response> {
        ...
        const deletedLogin = await deleteLogin.execute(
            id
        );
    ...
    public async getOne(req: Request, res: Response): Promise<Response> {
        ...
        const gotOneLogin = await getOneLogin.execute(
            id
        );
    ...
    const updatedLogin = await updateLogin.execute({
      ...
      tipoUsuario,
        ...
        return res.json(updatedLogin).status(201).send("Captura alterada com sucesso");
    }
}